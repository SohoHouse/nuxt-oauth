...
Handler.prototype.saveData = async function saveData (token) {
  ...
  const newToken = pickTokenProps(token)
  const sessionToken = pickTokenProps(this.getSessionToken())
  const updatedToken = Object.assign({}, sessionToken, newToken)
  const { accessToken } = updatedToken
