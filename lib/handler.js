...
Handler.prototype.authenticate = async function authenticate () {
  ...
  try {
    ...
    var data = { expires_in: 30*60 }


    if(expires)
    {
        // client-oauth2 has a bug in handling dates passed in through data
        // see issue https://github.com/mulesoft-labs/js-client-oauth2/issues/106
        // so we need to pass duration, rathern than a date
        data.expires_in = expires
        if (typeof expires === 'number') {
          data.expires_in = expires
        } else {
          
          data.expires_in = (Date.parse(expires) - Date.now())/1000
        }
    } 

    const token = await this.auth.createToken(accessToken, refreshToken, data, 'bearer')
