...
Handler.prototype.checkRequestAuthorization = async function checkRequestAuthorization () {
  ...
  try {
    ...
    if (existingToken) {
      ...
      errorLog("nuxt-oauth - existing token not found")
    }
  } catch (e) {
...
Handler.prototype.saveData = async function saveData (token) {
  ...
  if (!token) {
    errorLog("nuxt-oauth - no access token found. resetting")
...
Handler.prototype.authenticate = async function authenticate () {
  ...
  if (!accessToken) {
    errorLog("nuxt-oauth - no access token found.")
    return null
  }
...
Handler.prototype.useRefreshToken = async function useRefreshToken () {
  ...
  if (!accessToken || !refreshToken) {
    errorLog("nuxt-oauth - No access token or refresh token.")
    return null
  }
...
Handler.prototype.extractToken = function extractToken () {
  ...
  if (!authorization) {
    errorLog("nuxt-oauth - No authorization header found.")
    return null
  }
