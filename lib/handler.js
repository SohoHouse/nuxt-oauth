...
Handler.prototype.checkRequestAuthorization = async function checkRequestAuthorization () {
  ...
  const sessionToken = this.req[this.opts.sessionName]
...
/**
 * Appends the token to the session variable to make it
 * accessible during client side requests
 * 
 * TODO: Adding the whole token to the encrypted session token is a
 * potential security hole and should be addressed
 *
 * @param { Object } token 
 * @returns 
 */
...
Handler.prototype.authenticate = async function authenticate () {
  ...
  const { accessToken, refreshToken, expires } = this.req[this.opts.sessionName]
...
Handler.prototype.useRefreshToken = async function useRefreshToken () {
  ...
  const { accessToken, refreshToken } = this[this.opts.sessionName]
